...
@section('content')
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:18px;">
        ...
        {{-- Account tools --}}
        <div class="admin-card" style="grid-column:1 / -1; margin-top:18px; display:flex; align-items:center; justify-content:space-between; gap:16px;">
            <div>
            ...
            <button type="submit"
                    ...
                    onclick="return confirm('Send a password reset email to {{ $customer->email }}?');">
                Send Password Reset
            </button>
        </div>
        ...
        @if (session('status'))
        <div class="admin-card" style="grid-column:1 / -1; border-left:4px solid #2CBFAE; margin-top:12px;">
            {{ session('status') }}
        </div>
        @endif
...
<form id="resetForm"
      action="{{ route('admin.customers.sendReset', $customer->id) }}"
      method="POST"
      style="display:none;">
    @csrf
</form>
